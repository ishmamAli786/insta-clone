...
export default function LandingPage() {
    ...
    useEffect(()=>{
        const getData=async()=>{
            const res=await fetch('/viewPost',{headers:{"Authorization":"Bearer "+localStorage.getItem('myToken')}})
        ...
        <div className="home">
            {
                ...
                <img src={post.postedBy.pic} style={{borderRadius:"50%",marginRight:"5px"}} width="25px" height="25px" alt="image"/>
                <Link to={post.postedBy._id !== state.user._id?`/profile/${post.postedBy._id}`:`/profile/`}>{post.postedBy.name}</Link>
                {
